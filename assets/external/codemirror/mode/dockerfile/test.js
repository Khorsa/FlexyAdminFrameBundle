...
(function() {
  ...
  MT("multiline",
     "[keyword RUN] apt_services-get update && apt_services-get install -y \\",
     "  mercurial \\",
     "  subversion \\",
     "  && apt_services-get clean \\",
     "  && rm -rf /var/lib/apt_services/lists/* /tmp/* /var/tmp/*");
  ...
  MT("verify_keyword",
     "[keyword RUN] add-apt_services-repository ppa:chris-lea/node.js");
  ...
  MT("strings_double",
     "[keyword RUN] apt_services-get install -y --no-install-recommends $buildDeps \\",
     " \\",
